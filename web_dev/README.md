### What is a Webpage
* When a user opens a link in a <i><b>browser</i></b>, the browser sends a data <i><b>GET request</i></b> to a web <i><b>server</i></b> and then receives <i><b>responses</i></b> codes from the web server.
* Some codes are in HTML (HyperText Markup Language) format, which dictates the rendering of the requested webpage in the browser.
  ...
  * Codes within \<style\> tag are written in CSS (Cascading Style Sheets) format, which customizes the appearance of other tags in \<body\> tag. For example, define the size of picture.
  * Codes within \<script\> tag are <i>Javascript</i>, which would make other tags reactive/dynamic. For example, remove picure when it is clicked,  
* Other codes are external content, which would be consumed in tags.
  * A picture, code of which is fetched from <b>http://pet.pic/dog.jpg</b>, would appear at tag \<img\>'s position.  
* Take-away: User's browser gets codes from a server and renders them to be a webpage.
### What is a Website
...
* For example, when a user comments a video on youtube's website, the frontend code in user's browser is responsible for sending the comment in a <i><b>POST request</i></b> to the web server and backend code will handle user's post on youtube's web server and <i><b>responds</i></b> to user's browser with result of commenting. 
* Take-away: Frontend code requests and backend code responds.
### Document Object Model
* User browser construct a <i><b>DOM</i></b> (Document Object Model) based on webpage's HTML code in a branchy arrangement, so that tags within a tag become <i><b>child elements</i></b> of the <i><b>parent element</i></b>. For example, both \<head\> and \<body\> tags become child elements of parent element came from \<html\> tag.
* DOM is an interface for element manipulation via javascript programming. For example, frontend javascript code can request new video recommendation video list and update a bunch of elements every minite so that user's browser refreshes the video list automatically. In this way, the HTML becomes dynamic.
* Take-away: DOM is manipulated by javascript dynamically according to backend's response.
...
* Developers usually resort to frameworks, which provide DOM manipulator and request handler, to speed up the web development.
...
* @vue/cli is for website (more than webpage) development.
  ...
  * Used javascript library will be bundled together so the website becomes independent. (The site works in a restricted network without internet.) 
